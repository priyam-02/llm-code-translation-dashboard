...
interface VariationByLanguageChartProps {
  data: VariationData[];
  filters: Filters;
}

export default function VariationByLanguageChart({
  data,
  filters,
}: VariationByLanguageChartProps) {
  ...
  return (
    <div className="glass rounded-2xl shadow-lg p-8 border border-white/20 card-hover fade-in">
      <h3 className="text-2xl font-bold text-gray-900 mb-2">
        Code Variation by Target Language
      </h3>
      <p className="text-sm text-gray-600 mb-6">
        Cyclomatic complexity and code length variations across target
        languages.
        {showSortingNote && <span className="italic"> Sorted by ΔSLoC.</span>}
      </p>
      <ResponsiveContainer width="100%" height={500}>
        ...
        >
          ...
          <Bar
            ...
            maxBarSize={80}
          />
          <Bar dataKey="deltaLOC" fill="#f59e0b" name="ΔSLoC" maxBarSize={80} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
